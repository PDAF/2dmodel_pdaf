...
With the tutorial model, we simulate a real case. The model itself is independent of PDAF and uses it own compile scheme. The online-coupled DA code extends the model. For compiling the online-coupled model, the Makefile of the model is extended to also compile the user code for the online coupling, while the PDAF library is compiled separately and just linked to the online-coupled assimilative model. In contrast, the offline-coupled DA code does not directly use the model and the Makefile is more tightly integrated with the PDAF library. Thus, it uses the compile definitions file from the PDAF release and can also initiate the compilation of the PDAF library.
...
The model setup uses a domain size of 18x36 grid points and fields that are initialized from a sine wave. The true state is rotated by 45 degrees. For the DA one can either directly read ensemble files, or one can use second-order exact smapling from a covariance matrix file. The ensemble for fieldA is generated by rotating a sine wave and varying its wavelenght. The ensemble for fieldB is generated by shifting the model analogous to the model dynamics. The available input files allow to run experiments with ensembles up to 20 states and up to 99 time steps. 

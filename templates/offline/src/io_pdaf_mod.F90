...
!!
  subroutine read_ensstate_pdaf(member, state_p)
    ...
    ! *** Read state for ensemble index 'member'

    ! Template reminder - delete when implementing functionality
    WRITE (*,*) 'TEMPLATE io_pdaf_mod.F90: Implement reading an ensemble state vector from file(s)!'
...
!! singular values.
!!
!! This routine is generic and should generally work
!! when the covariance matrix file is generated from
!! the generate_covar template.
!!
...
!        ! Read mean state
!        do fid = 1, n_fields
!           call nfcheck( NF90_INQ_VARID(ncid, 'mean'//trim(sfields(fid)%name), id_state))
! 
...
!        do j = 1, dim_ens-1
!           do fid = 1, n_fields
!              call nfcheck( NF90_INQ_VARID(ncid, 'u_svd'//trim(sfields(fid)%name), id_eofs))
! 
!              startv(3) = j
!              countv(3) = 1
!              startv(2) = 1+offset_x_p
...
! *** Write ensemble mean state fields ***
    ...
    if (write_state) then
       do fid = 1, n_fields
          ! Set file name
          filestr = 'state'//trim(sfields(fid)%name)
...
! *** Write ensemble state fields ***
    ...
    if (write_ens) then
       do member = 1, dim_ens
          ...
          do fid = 1, n_fields
             ! Set file name
             filestr = 'ens'//trim(sfields(fid)%name)//'_'//trim(ensstr)
             filename = trim(filestr)//'_step'//trim(stepstr)//'_'//trim(anastr)//'.nc'
...
! *** Write ensemble variance fields ***
    ...
    if (write_var) then
       do fid = 1, n_fields
          ! Set file name
          filestr = 'variance'//trim(sfields(fid)%name)
          filename = trim(filestr)//'_step'//trim(stepstr)//'_'//trim(anastr)//'.nc'
